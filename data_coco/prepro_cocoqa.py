...
def main(params):
    ...
    f.create_dataset("ques_val", dtype='uint32', data=ques_test)

    f.create_dataset("answers", dtype='uint32', data=ans_train)
    f.create_dataset("ans_val", dtype='uint32', data=ans_test)
    
    f.create_dataset("ques_id_train", dtype='uint32', data=question_id_train)
    f.create_dataset("ques_id_val", dtype='uint32', data=question_id_test)

    f.create_dataset("img_pos_train", dtype='uint32', data=img_pos_train)
    f.create_dataset("img_pos_val", dtype='uint32', data=img_pos_test)

    f.create_dataset("split_train", dtype='uint32', data=split_train)
    f.create_dataset("split_val", dtype='uint32', data=split_test)

    f.create_dataset("ques_len_train", dtype='uint32', data=ques_length_train)
    f.create_dataset("ques_len_val", dtype='uint32', data=ques_length_test)
    ...
    out['unique_img_val'] = unique_img_test
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_train_json', default='cocoqa_raw_train.json', help='input json file to process into hdf5')
    parser.add_argument('--input_test_json', default='cocoqa_raw_test.json', help='input json file to process into hdf5')

    parser.add_argument('--output_json', default='./cocoqa_data_prepro.json', help='output json file')
    parser.add_argument('--output_h5', default='./cocoqa_data_prepro.h5', help='output h5 file')
