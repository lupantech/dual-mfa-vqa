...
def build_vocab_question(imgs, imgs_val, params):
...
def get_top_answers(imgs, imgs_val, params):  # [PAN]
    counts = {}
    for img in imgs:
        ans = img['ans'] 
        counts[ans] = counts.get(ans, 0) + 1
    for img in imgs_val:  # [PAN]
        ans = img['ans'] 
        counts[ans] = counts.get(ans, 0) + 1
...
def main(params):
    ...
    # {u'ques_id': 2845511, u'MC_ans': [u'blue', u'pizza', u'1', u'no', u'forest', u'red', u'white', u'casino', u'yes', u'macaco de perigo', u'2', u'in truck', u'in shack', u'lt 35', u'3', u'green', u'not', u'4'], u'question': u'Is this their natural habitat?', u'img_path': u'train2014/COCO_train2014_000000284551.jpg', u'ans': u'no'}

    #imgs_train = imgs_train[:5000]
    #imgs_test = imgs_test[:5000]
    # get top answers
    top_ans = get_top_answers(imgs_train,imgs_val,params) # [PAN]
    ...
    imgs_train, vocab = build_vocab_question(imgs_train, imgs_val, params) # [PAN]
    print 'number of words in question vocab =' , len(vocab) # 15030
    ...
    print "imgs_train = ",len(imgs_train) # 334554
    print "unique_img_train = ",len(unique_img_train) # 123070

    print "imgs_test = ",len(imgs_test) # 60864
    print "unique_img_test = ",len(unique_img_test) # 20288
    
    print "imgs_val = ",len(imgs_val) # 109627
    print "unique_img_val = ",len(unique_img_val) # 40443
    
    # for i in range(len(unique_img_val)):
    #     print "%d th unique_img_val %s" %(i, unique_img_val[i])
    # for i in range(len(img_pos_val)):
    #     print "%d th img_pos_val %s" %(i, img_pos_val[i])
    ...
    #     print "ans_val[i]=",ans_val[i] # [1,2000]
    ...
    f.create_dataset("ques_id_val", dtype='uint32', data=question_id_val)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_val_json', default='./vqa_raw_val.json', help='input json file to process into hdf5')
    parser.add_argument('--num_ans', default=2000, type=int, help='number of top answers for the final classifications.')
